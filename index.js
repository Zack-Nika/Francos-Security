/**
 * index.js - Franco's Security [SHIELD]
 * 
...
// Log messages to the dedicated channel "suspicious-actions-logs".
function logToChannel(guild, message) {
  if (!botApproved) return;
  const logChannel = guild.channels.cache.find(c => c.name === 'suspicious-actions-logs' && c.type === ChannelType.GuildText);
  if (logChannel) {
...
async function ensureLogChannel(guild) {
  let logChannel = guild.channels.cache.find(c => c.name === 'suspicious-actions-logs' && c.type === ChannelType.GuildText);
  if (!logChannel) {
    try {
      logChannel = await guild.channels.create({
        name: 'suspicious-actions-logs',
...
// Send a custom DM to the owner after the bot is approved.
...
Franco's Security [SHIELD] is now LIVE and protecting your server like a digital fortress. Here is what I can do:

Core Features:
- Owner Approval: I am only activated when you approve me.
- Anti-Nuke & Anti-Spam: I detect and block malicious actions automatically.
- Trust Score System: Users start with a score of 100. Misbehavior drops this score. If it falls too low, I ban the user.
- Scheduled Backups: I back up essential data every 2 hours (or you can trigger one immediately using /backup).
- Auto-Healing: If critical roles or settings are tampered with, I will restore them automatically.
- Whitelist Management: Use /whitelist @user and /unwhitelist @user to manage trusted users.
- Instant Ban Policy: Any message containing malware/threat keywords is immediately met with a ban.
- Role Protection: Tampering with security roles triggers auto-healing and a ban.

New Suspicious Account Check:
- If a new member's account is less than 30 days old, I send a DM challenge to verify they are human.
  - A correct reply (e.g., "I'm human") will keep their trust score intact.
  - No or incorrect reply will lower their trust score by 25 points.

What You Can Do:
- Monitor the "suspicious-actions-logs" channel for logs.
- Check trust scores with /trustscore @user.
- Use /restore to revert any unwanted changes.

Relax and let Franco's Security [SHIELD] keep your digital realm safe.
    `;
    owner.send(message);
  } catch (error) {
...
client.once(Events.ClientReady, () => {
  ...
  if (!botApproved) {
    client.users.fetch(ownerId).then(user => {
      ...
      user.send({ content: "Please approve Franco's Security [SHIELD] to activate the bot:", components: [row] })
          .catch(console.error);
    });
  }
...
client.on(Events.GuildMemberAdd, async member => {
  ...
  // Define threshold: accounts younger than 30 days are suspicious.
  ...
  if (accountAge < suspiciousThreshold) {
    try {
      // Send DM challenge.
      const dm = await member.user.send("Welcome! Your account is less than 30 days old. Please reply with 'I'm human' within 60 seconds to verify that you are not a self-bot.");
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isButton()) {
    if (interaction.customId === "approve_bot") {
      ...
      await interaction.update({ content: "Bot Approved! Franco's Security [SHIELD] is now active!", components: [] });
